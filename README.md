...
- 自动建立联系人并统计双向往来差额
- 按事件、年份、联系人和人情类型查看数据
- 账户设置支持修改昵称、浅色/深色主题和随机背景开关
- 响应式 H5 界面默认使用 `https://img.lileyi.de/random?dir=random&type=img&orientation=auto` 随机背景，加载失败时自动保留备用背景
...
7. 切换深色模式、更换随机背景，刷新页面确认数据和外观设置均正常保留。
