...
- 事件、联系人、礼金明细、联系人账本和操作日志均采用服务端分页与滚动加载，搜索和筛选由后端执行
- 自动建立联系人并统计双向往来差额
- 按事件、年份、联系人和人情类型查看数据
- 账户设置支持修改昵称和浅色/深色主题
- 当前账号数据可导出为版本化备份，支持校验、密码确认和事务覆盖恢复
...
事件、联系人、礼金记录、联系人账本和操作日志接口统一接受 `page`、`pageSize` 参数。列表数据仍保留在原有 `data`/`records` 字段中，并额外返回：
...
事件和联系人搜索使用后端 `keyword` 参数，联系人还可使用 `relation`，事件可使用 `hosted`。备份导入导出始终处理当前账号的完整逻辑数据，不使用分页。

## 用户数据备份

登录后在首页账户设置中打开“数据备份与恢复”：

- 导出会下载 `.giftledger` 文件，只包含当前账号的联系人、事件、礼金记录和操作日志。
- 备份不包含用户 ID、登录手机号、密码哈希和 JWT，可恢复到另一个已登录账号。联系人业务资料中的号码会正常备份。
- 导入会先校验备份版本、字段和关联关系，正式恢复时需要输入当前账号密码。
- 恢复会在一个 SQLite 事务中覆盖当前账号的现有账簿；操作前请先导出当前数据。

用户级逻辑备份不能代替 VPS 级 SQLite Volume 灾备，整库备份方法见下文“数据备份与恢复”。
...
```text
ghcr.io/945967063/gift-ledger-h5:latest
```

推送 `master` 或 `v*` 标签时，[Docker 发布流程](.github/workflows/docker-publish.yml)会自动构建并发布同时包含 Vue 页面和 Express API 的 `linux/amd64`、`linux/arm64` 单镜像。`latest` 跟随 `master`，版本标签和 `sha-*` 标签可用于固定版本或回滚。GitHub Package 必须设为 **Public**，VPS 才能免登录拉取。
...
浏览器 → Cloudflare → 宿主机 Nginx/Certbot :443
       → http://127.0.0.1:8090 → 应用容器 Node.js :3000
       → Vue 静态页面 / Express API → SQLite Volume
```
...
| `8090` | `127.0.0.1`  | 宿主机 Nginx 到应用容器 `3000` 的本地端口映射 |
| `3000` | 应用容器内部 | Node.js 同时提供 Vue 页面和 API，不映射到公网 |
...
同源部署时 `CORS_ORIGIN` 保持为空。应用容器会在生产启动时拒绝空值或不足 32 位的 `JWT_SECRET`。
...
依次验证容器内 API、Docker 端口、宿主机 Nginx 和公网链路：
...
8. 在账户设置导出备份，重新选择该文件并确认预览数量正确。

### 6. 后续更新与回滚

如果 VPS 当前还在运行 `gift-ledger-frontend` 和 `gift-ledger-backend` 两个旧容器，首次升级到单容器版本时先执行：

```bash
cd /opt/gift-ledger-h5

# 使用旧 Compose 停止两个容器；不要添加 -v
...
显式保留的 `gift_ledger_sqlite_data` Volume 会被新 `app` 容器继续挂载。先停止旧容器可避免新旧 Node.js 进程同时写入 SQLite。
...
| 现象                    | 优先检查                                                         |
| ----------------------- | ---------------------------------------------------------------- |
| GHCR `denied`           | `gift-ledger-h5` Package 是否为 Public，镜像标签是否存在         |
| 本机 `8090` 无响应      | `docker compose ps`、应用日志、`.env` 中的 `JWT_SECRET`          |
...
SQLite 只适合单应用实例，不要让多个容器同时写入同一个 Volume。整库备份前停止应用，以便同时保存数据库和 WAL 状态：
...
确认备份文件存在且大小合理，并将其复制到 VPS 之外。恢复前必须停止应用，先为当前 Volume 额外制作快照，再将指定备份完整恢复；不要在运行中的 SQLite 数据库上直接覆盖文件。
...
- `pnpm check` 全部通过
- GitHub Actions 成功发布单一 `gift-ledger-h5` 应用镜像
